...
class Student(db.Model):
    ...
    def remaining_balance(self):
        ...
        return self.total_course_price - self.total_paid
...
def format_arabic_date(date_obj):
    """Format date as: السبت، 4 يونيو 2025"""
    arabic_months = {
...
def add_group():
    ...
    # Add schedules - now using individual day fields
    selected_days = request.form.getlist('days[]')
    
    for day in selected_days:
        # Get the time fields for this specific day
        day_prefix = {
            'السبت': 'sat',
            'الأحد': 'sun', 
            'الاثنين': 'mon',
...
def reports():
    ...
    # Calculate pending payments based on new group-based pricing
    pending_payments = 0
    students = Student.query.all()
    ...
    total_groups_revenue = sum(group.price for group in Group.query.all())
...
def edit_student(student_id):
    try:
        ...
        student.registration_date = datetime.strptime(request.form['registration_date'], '%Y-%m-%d')
...
def edit_group(group_id):
    ...
    group.instructor_id = int(request.form['instructor_id'])
    group.max_students = int(request.form['max_students'])
    
    # Note: For simplicity, we're only updating basic group info in edit mode
    # Schedule editing would require a more complex interface
    
    db.session.commit()
    flash('تم تحديث بيانات المجموعة بنجاح', 'success')
    return redirect(url_for('groups'))
...
def group_details(group_id):
    ...
    total_expected_revenue = sum(student.total_course_price for student in students)
...
def export_full_backup():
    ...
    try:
        ...
        student_headers = ['#', 'اسم الطالب', 'الهاتف', 'العمر', 'الموقع', 'المدرس', 'المجموعات', 'إجمالي السعر', 'المدفوع', 'المتبقي', 'تاريخ التسجيل']
        for col, header in enumerate(student_headers, 1):
